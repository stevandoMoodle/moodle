...
export const setupForElementId = ({elementId, options}) => {
    return setupForTarget(document.querySelector(`#${elementId}`), options);
};
...
const getStandardConfig = (target, tinyMCE, plugins) => {
    const lang = document.querySelector('html').lang;
    return {
...
export const setupForTarget = async(target, options = {}) => {
    ...
    const instanceConfig = getStandardConfig(target, tinyMCE, pluginNames);
