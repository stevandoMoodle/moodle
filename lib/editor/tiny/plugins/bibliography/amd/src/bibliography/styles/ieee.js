...
export default class Ieee extends Base {
    ...
    deleteBibliography = (bio) => {
        const list = bio.editor.dom.select(`[class="${bio.deleteBio.dataset.id}"]`);
        const index = parseInt(bio.deleteBio.dataset.index);
        if (list) {
    ...
    generateBibliography = async(bio) => {
        if (this.checkRequiredFields(bio)) {
            if (bio.create) {
                ...
                window.console.log('index', bio.updateIndex);
                ...
                if (index === 0) {
                    ...
                    if (currentElement && elementClass && elementClass.includes('id-bibliography-list-')) {
                        ...
                        window.console.log('second item');
                        bio.editor.dom.add(
                            bioList[0],
                            'p',
