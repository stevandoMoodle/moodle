...
const exampleMap = {
    ...
    isFirstEditionExample: 'Yes/No',

    languageExample: 'English',
...
export const getExample = () => {
    ...
    form.forEach(field => {
        ...
        if (example) {
            addExampleListener(field.id, exampleMap[example]);
        }
    });
};

const addExampleListener = (element, example) => {
    if (Array.isArray(element)) {
        element.forEach(el => {
            if (byId(el)) {
                const exampleEl = byId(el);
                const handler = createExampleHandler.bind(null, example);
