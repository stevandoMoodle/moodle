...
if ($ADMIN->fulltree) {
    ...
    if (!$isvalid && !isset($_SERVER['HTTPS'])) {
    ...
    $setting = new admin_setting_configduration('tiny_recordrtc/audiotimelimit', $name, $desc, DEFAULT_TIME_LIMIT);
    ...
    $setting = new admin_setting_configduration('tiny_recordrtc/videotimelimit', $name, $desc, DEFAULT_TIME_LIMIT);
    $setting->set_validate_function(function(int $value): string {
        if ($value <= 0) {
            return get_string('timelimitwarning', 'tiny_recordrtc');
        }
