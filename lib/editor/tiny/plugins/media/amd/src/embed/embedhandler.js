...
export class EmbedHandler {
    ...
    getContextForMediaHTML = (tab, details) => {
        ...
        let controls = details.querySelector(Selectors.EMBED.elements.mediaControl).checked;
        let autoplay = details.querySelector(Selectors.EMBED.elements.mediaAutoplay).checked;

        // Set autoplay to false if controls is false.
        if (!controls) {
            autoplay = false;
        }

        // Set controls to true if autoplay is true.
        if (autoplay) {
            controls = true;
        }
