...
export class EmbedPreview extends MediaBase {
    ...
    registerMediaDetailsEventListeners = async() => {
        ...
        // Handles show controls and autoplay media.
        const mediaAutoplay = this.root.querySelector(Selectors.EMBED.elements.mediaAutoplay);
        const mediaControl = this.root.querySelector(Selectors.EMBED.elements.mediaControl);
        if (mediaAutoplay && mediaControl) {
            mediaAutoplay.addEventListener('change', (e) => {
                if (e.target.checked) {
                    mediaControl.checked = true;
                }
            });

            mediaControl.addEventListener('change', (e) => {
                if (!e.target.checked) {
                    mediaAutoplay.checked = false;
                }
            });
        }
    };
