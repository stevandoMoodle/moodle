...
const AICourseAssist = class {
    ...
    disableActionControls() {
        const actionControls = document.querySelector(Selectors.ELEMENTS.ACTION_CONTROLS);
        if (actionControls) {
            actionControls.setAttribute('disabled', 1);
        }
    }

    /**
     * Enable the action buttons and focus on it.
     */
   enableActionControls() {
       const actionControls = document.querySelector(Selectors.ELEMENTS.ACTION_CONTROLS);
        if (actionControls) {
            actionControls.removeAttribute('disabled');
            actionControls.focus();
        }
    }
            ...
            try {
                ...
                if (responseObj.error) {
                    ...
                    if (!this.isRequestCancelled()) {
                        ...
                        let generatedContent = AIHelper.replaceLineBreaks(responseObj.generatedcontent);
                        generatedContent = AIHelper.replaceMarkdown(generatedContent);
