...
Feature: AI course assist features
  ...
  Scenario: AI features dropdown is not visible when only one feature is enabled
    Given the following config values are set as admin:
      | explain_text | | aiprovider_openai |
    When I am on the "PageName1" "page activity" page logged in as teacher1
  ...
  Scenario: AI features is not available if placement is not enabled
    Given the following config values are set as admin:
      | enabled | | aiplacement_courseassist |
    When I am on the "PageName1" "page activity" page logged in as teacher1
    Then "AI features" "button" should not exist

  Scenario: AI features is not available if provider is not enabled
    Given the following config values are set as admin:
      | enabled | | aiprovider_openai |
    When I am on the "PageName1" "page activity" page logged in as teacher1
    Then "AI features" "button" should not exist

  Scenario: AI features is not available if provider action is not enabled
    Given the following config values are set as admin:
  ...
  Scenario: AI features is not available if placement action is not enabled
    Given the following config values are set as admin:
      | summarise_text | | aiplacement_courseassist |
      | explain_text   | | aiplacement_courseassist |
    When I am on the "PageName1" "page activity" page logged in as teacher1
    Then "AI features" "button" should not exist

  Scenario: AI features is not available if the user does not have permission
